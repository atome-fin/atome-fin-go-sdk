...
| `atomefin/bill` | `Service` with `Bills` / `BillDetail` / `BillsUnpaid` + `BillsAll` auto-pagination; types `Bill`, `BillDetail`, `BillMainOrder`, `BillDiscounts`, `Discount`, `DiscountDetail` + `OverdueStatus` enum. |
...
| `GET` | `/billDetail` | partner → atome-fin | `bill.New(c).BillDetail(ctx, billID, externalReferenceUID)` | `billId` + `externalReferenceUid` query | `*bill.BillDetailResponse` | full single-bill view including `mainOrders` + `discounts` |
