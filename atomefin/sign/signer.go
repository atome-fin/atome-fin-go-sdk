...
//
// The signing scheme is **RSASSA-PKCS#1 v1.5 over SHA-256** ("RSA2" in the
// spec wording). The Atome gateway only supports PKCS#1 v1.5 — there is no
// PSS branch. Earlier SDK versions shipped opt-in `WithSaltedPSS` /
// `WithVerifierSaltedPSS` scaffolding behind a misreading of the spec
// ("Encrypt the signature with salt if necessary" — re-read in v0.3 as
// referring to the encrypt-cert pair, not PSS); v0.7.0 removed the PSS
// scaffolding because dead options invite partner confusion.
package sign
...
// rsa2Signer is the RSA-PKCS#1 v1.5 implementation. Immutable after
...
// RSA-2048-or-larger private key using RSASSA-PKCS#1 v1.5 padding.
//
// PKCS#1 v1.5 is the only signing scheme the Atome gateway accepts; the
// v0.6.x `WithSaltedPSS` opt-in was removed in v0.7.0 (see package doc
// for the history).
